...
export const useCurioDataStore = defineStore("curioData", () => {
  ...
  const createDataAccessor = <T>(method: string, params: unknown[] = []) => {
    const key = `${method}:${JSON.stringify(params)}`;
    ...
    return {
      ...
      hasData: computed(
        () =>
          getCachedData<T>(key)?.data !== null &&
          getCachedData<T>(key)?.data !== undefined,
      ),
      lastUpdated: computed(() => getCachedData<T>(key)?.lastUpdated || 0),
      refresh: () => refreshData<T>(method, params),
      startPolling: (interval?: number) =>
        startPolling(method, params, interval),
