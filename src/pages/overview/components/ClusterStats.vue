...
const { data: taskStats, loading: tasksLoading, error: tasksError } = useCachedQuery<HarmonyTaskStat[]>('HarmonyTaskStats', [], {
  pollingInterval: 30000
})

const { data: storageStats, loading: storageLoading, error: storageError } = useCachedQuery<StorageUseStat[]>('StorageUseStats', [], {
  pollingInterval: 30000
})

const { data: actors, loading: actorsLoading, error: actorsError } = useCachedQuery<ActorSummaryData[]>('ActorSummary', [], {
  pollingInterval: 30000
})

const loading = computed(() => machinesLoading.value || tasksLoading.value || storageLoading.value || actorsLoading.value)
const error = computed(() => machinesError.value || tasksError.value || storageError.value || actorsError.value)

const isInitialLoading = computed(() => loading.value && (!machines.value && !taskStats.value && !storageStats.value && !actors.value))
...
const taskSuccessRate = computed(() => {
  ...
  const totalSuccess = taskStats.value.reduce((sum, task) => sum + task.TrueCount, 0)
  const totalTasks = taskStats.value.reduce((sum, task) => sum + task.TotalCount, 0)
...
const storageUsagePercentage = computed(() => {
  ...
  const totalCapacity = storageStats.value.reduce((sum, stat) => sum + stat.Capacity, 0)
  const totalAvailable = storageStats.value.reduce((sum, stat) => sum + stat.Available, 0)
...
const totalStorageCapacity = computed(() => {
  ...
  const totalTB = storageStats.value.reduce((sum, stat) => sum + stat.Capacity, 0) / (1024 * 1024 * 1024 * 1024)
...
const totalActorBalance = computed(() => {
  ...
  actors.value.forEach(actor => {
    const balance = parseFloat(actor.ActorBalance.replace(/[^\d.-]/g, '') || '0')
...
const totalActorAvailable = computed(() => {
  ...
  actors.value.forEach(actor => {
    const available = parseFloat(actor.ActorAvailable.replace(/[^\d.-]/g, '') || '0')
...
<template>
  <div v-if="!isInitialLoading" class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <!-- Online Machines -->
    <KPICard
    ...
    <!-- Storage Usage -->
    <div class="bg-base-100 border border-base-300 rounded-lg p-4 h-[180px] flex flex-col">
      <div class="flex items-start justify-between mb-3">
        <div class="flex-1">
          <div class="text-sm font-medium text-base-content/60 mb-1">
            Storage Usage
          </div>
          <div class="text-3xl font-bold text-base-content tracking-tight mb-2">
            {{ storageUsagePercentage }}%
          </div>
          <div class="text-xs font-medium" 
               :class="storageUsagePercentage > 90 ? 'text-error' : storageUsagePercentage > 80 ? 'text-base-content/60' : 'text-success'">
            {{ totalStorageCapacity }} total capacity
          </div>
        </div>
        <div 
          class="rounded-lg p-2.5 ml-3"
          :class="storageUsagePercentage > 90 ? 'bg-error/10 text-error' : storageUsagePercentage > 80 ? 'bg-warning/10 text-warning' : 'bg-success/10 text-success'"
        >
      ...
      <div class="flex-1 flex flex-col justify-end">
        <div class="space-y-2">
          <template v-if="storageStats && storageStats.length > 0">
            <div v-for="stat in storageStats.slice(0, 3)" :key="stat.Type" class="flex justify-between items-center">
              <span class="text-xs text-base-content/70">{{ stat.Type }}</span>
              <div class="text-right">
                <span class="text-xs font-medium">{{ Math.round((1 - stat.Available / stat.Capacity) * 100) }}%</span>
              </div>
            </div>
          </template>
          <div v-else class="text-xs text-base-content/50">No detailed breakdown available</div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-base-100 border border-base-300 rounded-lg p-4 h-[180px] flex flex-col">
      <div class="flex items-start justify-between mb-3">
        ...
        <div class="rounded-lg p-2.5 ml-3 bg-accent/10 text-accent">
      ...
      <div class="flex-1 flex flex-col justify-end">
        <div class="space-y-2">
          <template v-if="actors && actors.length > 0">
            ...
            <div class="flex justify-between items-center">
              <span class="text-xs text-base-content/70">Recent Wins (7d)</span>
              <span class="text-xs font-medium">{{ totalWins7d }}</span>
            </div>
          </template>
          <div v-else class="text-xs text-base-content/50">No actor data available</div>
        </div>
      </div>
    </div>
    ...
    <div class="col-span-2 lg:col-span-2">
      <div class="bg-base-100 border border-base-300 rounded-lg p-4 h-[180px] flex flex-col">
        ...
        <div class="flex-1 flex flex-col">
          <div class="space-y-3 flex-1">
            <div class="flex items-center justify-between">
              ...
              <div class="text-right">
                ...
                <span class="text-xs text-base-content/60 ml-1">machines</span>
              </div>
            </div>
            
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <ExclamationTriangleIcon class="size-4 text-error" />
                <span class="text-sm">Offline</span>
              </div>
              <div class="text-right">
                <span class="font-semibold">{{ totalMachines - onlineMachines }}</span>
                <span class="text-xs text-base-content/60 ml-1">machines</span>
              </div>
            </div>
          </div>
          ...
          <!-- Health indicator -->
          <div class="pt-2 border-t border-base-300 mt-auto">
            <div class="flex justify-between items-center mb-2">
              <span class="text-sm text-base-content/60">Cluster Health</span>
              <span class="text-sm font-medium">{{ machineHealthPercentage }}%</span>
            </div>
            <div class="w-full bg-base-200 rounded-full h-2">
              <div 
                class="h-2 rounded-full transition-all duration-500"
                :class="machineHealthPercentage > 80 ? 'bg-success' : machineHealthPercentage > 60 ? 'bg-warning' : 'bg-error'"
  ...
  <div v-else class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <div v-for="i in 6" :key="i" class="animate-pulse">
      <div class="bg-base-100 border border-base-300 rounded-lg p-4 h-24">
  ...
  <div v-if="error" class="text-center py-8 text-error">
    <div class="text-lg mb-2">📊 Stats Error</div>
