...
const { data: storeBreakdown, loading: breakdownLoading, error: breakdownError, hasData: breakdownHasData } = 
  ...
  if (summary.value && summary.value.length > 0) {
    const hasDetailedBreakdown = 
      storeBreakdown.value && (
        ...
        (storeBreakdown.value.length === 1 && storeBreakdown.value[0].type !== 'Other')
      )
...
const getAvailablePercentage = (capacity: number, available: number): string => {
          ...
          <template v-for="row in data" :key="row.Type">
            <tr>
              ...
              <td>
                <div class="space-y-1">
                  <div class="text-sm">
                    {{ row.UseStr || (row.Capacity - row.Available) }} / {{ row.CapStr || row.Capacity }}
                  </div>
              ...
              <td>
                <progress 
                  class="progress w-56" 
                  :class="getProgressColor(parseFloat(getUsagePercentage(row.Capacity, row.Available)))"
            ...
            >
              ...
              <td class="text-sm">
                <div class="space-y-1">
                  ...
                  <div class="text-xs text-base-content/70">
                    {{ getAvailablePercentage(sub.capacity, sub.available) }}% available
                  </div>
                </div>
              </td>
              <td>
                <progress 
                  class="progress w-48" 
                  :class="getProgressColor(parseFloat(getUsagePercentage(sub.capacity, sub.available)))"
