...
function MarketSettingsPage() {
  ...
  const [pricingForm, setPricingForm] = useState({ name: "", minDur: 0, maxDur: 0, minSize: 0, maxSize: 0, price: 0, verified: false });
  const [showAddClient, setShowAddClient] = useState(false);
  const [clientForm, setClientForm] = useState({ name: "", active: true, wallets: "", peers: "", filters: "", maxDealsPerHour: 0, maxDealSizePerHour: 0, info: "" });
  const [showAddAllowDeny, setShowAddAllowDeny] = useState(false);
  const [allowDenyForm, setAllowDenyForm] = useState({ wallet: "", status: true });

  const handleAddPricing = useCallback(() => {
    if (!pricingForm.name.trim()) return;
    addPricingMutation.mutate([pricingForm.name, pricingForm.minDur, pricingForm.maxDur, pricingForm.minSize, pricingForm.maxSize, pricingForm.price, pricingForm.verified]);
    setPricingForm({ name: "", minDur: 0, maxDur: 0, minSize: 0, maxSize: 0, price: 0, verified: false });
    setShowAddPricing(false);
  }, [pricingForm, addPricingMutation]);

  const handleAddClient = useCallback(() => {
    if (!clientForm.name.trim()) return;
    const wallets = clientForm.wallets.split(",").map((s) => s.trim()).filter(Boolean);
    const peers = clientForm.peers.split(",").map((s) => s.trim()).filter(Boolean);
    const filters = clientForm.filters.split(",").map((s) => s.trim()).filter(Boolean);
    addClientMutation.mutate([clientForm.name, clientForm.active, wallets, peers, filters, clientForm.maxDealsPerHour, clientForm.maxDealSizePerHour, clientForm.info]);
    setClientForm({ name: "", active: true, wallets: "", peers: "", filters: "", maxDealsPerHour: 0, maxDealSizePerHour: 0, info: "" });
    setShowAddClient(false);
  }, [clientForm, addClientMutation]);

  const handleAddAllowDeny = useCallback(() => {
    if (!allowDenyForm.wallet.trim()) return;
    addAllowDenyMutation.mutate([allowDenyForm.wallet.trim(), allowDenyForm.status]);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={() => setShowAddPricing(true)}>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={() => setShowAddAllowDeny(true)}>
      ...
      {showAddPricing && (
        <Dialog open onOpenChange={() => setShowAddPricing(false)}>
          <DialogContent className="max-w-md" onClose={() => setShowAddPricing(false)}>
            <DialogHeader>
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input value={pricingForm.name} onChange={(e) => setPricingForm((f) => ({ ...f, name: e.target.value }))} placeholder="Filter name" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium">Min Duration (days)</label>
                  <Input type="number" value={pricingForm.minDur} onChange={(e) => setPricingForm((f) => ({ ...f, minDur: parseInt(e.target.value) || 0 }))} />
                </div>
                <div>
                  <label className="text-sm font-medium">Max Duration (days)</label>
                  <Input type="number" value={pricingForm.maxDur} onChange={(e) => setPricingForm((f) => ({ ...f, maxDur: parseInt(e.target.value) || 0 }))} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium">Min Size (bytes)</label>
                  <Input type="number" value={pricingForm.minSize} onChange={(e) => setPricingForm((f) => ({ ...f, minSize: parseInt(e.target.value) || 0 }))} />
                </div>
                <div>
                  <label className="text-sm font-medium">Max Size (bytes)</label>
                  <Input type="number" value={pricingForm.maxSize} onChange={(e) => setPricingForm((f) => ({ ...f, maxSize: parseInt(e.target.value) || 0 }))} />
                </div>
              </div>
              <div>
                <label className="text-sm font-medium">Price (attoFIL/GiB/Epoch)</label>
                <Input type="number" value={pricingForm.price} onChange={(e) => setPricingForm((f) => ({ ...f, price: parseInt(e.target.value) || 0 }))} />
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" checked={pricingForm.verified} onChange={(e) => setPricingForm((f) => ({ ...f, verified: e.target.checked }))} id="verified" />
                <label htmlFor="verified" className="text-sm font-medium">Verified deals only</label>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowAddPricing(false)}>Cancel</Button>
              <Button onClick={handleAddPricing} disabled={addPricingMutation.isPending || !pricingForm.name.trim()}>
        ...
        <Dialog open onOpenChange={() => setShowAddClient(false)}>
          <DialogContent className="max-w-md" onClose={() => setShowAddClient(false)}>
            ...
            <div className="space-y-3">
              <div>
                <label className="text-sm font-medium">Name *</label>
                <Input value={clientForm.name} onChange={(e) => setClientForm((f) => ({ ...f, name: e.target.value }))} placeholder="Filter name" />
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" checked={clientForm.active} onChange={(e) => setClientForm((f) => ({ ...f, active: e.target.checked }))} id="clientActive" />
                <label htmlFor="clientActive" className="text-sm font-medium">Active</label>
              </div>
              <div>
                <label className="text-sm font-medium">Wallets (comma-separated)</label>
                <Input value={clientForm.wallets} onChange={(e) => setClientForm((f) => ({ ...f, wallets: e.target.value }))} placeholder="f1..., f3..." />
              </div>
              <div>
                <label className="text-sm font-medium">Peer IDs (comma-separated)</label>
                <Input value={clientForm.peers} onChange={(e) => setClientForm((f) => ({ ...f, peers: e.target.value }))} />
              </div>
              <div>
                <label className="text-sm font-medium">Pricing Filters (comma-separated)</label>
                <Input value={clientForm.filters} onChange={(e) => setClientForm((f) => ({ ...f, filters: e.target.value }))} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium">Max Deals/Hour</label>
                  <Input type="number" value={clientForm.maxDealsPerHour} onChange={(e) => setClientForm((f) => ({ ...f, maxDealsPerHour: parseInt(e.target.value) || 0 }))} />
                </div>
                <div>
                  <label className="text-sm font-medium">Max Deal Size/Hour</label>
                  <Input type="number" value={clientForm.maxDealSizePerHour} onChange={(e) => setClientForm((f) => ({ ...f, maxDealSizePerHour: parseInt(e.target.value) || 0 }))} />
                </div>
              </div>
              <div>
                <label className="text-sm font-medium">Additional Info</label>
                <Input value={clientForm.info} onChange={(e) => setClientForm((f) => ({ ...f, info: e.target.value }))} />
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowAddClient(false)}>Cancel</Button>
              <Button onClick={handleAddClient} disabled={addClientMutation.isPending || !clientForm.name.trim()}>
      ...
      {showAddAllowDeny && (
        <Dialog open onOpenChange={() => setShowAddAllowDeny(false)}>
          <DialogContent className="max-w-sm" onClose={() => setShowAddAllowDeny(false)}>
            <DialogHeader>
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input value={allowDenyForm.wallet} onChange={(e) => setAllowDenyForm((f) => ({ ...f, wallet: e.target.value }))} placeholder="f0... or f1..." className="font-mono text-xs" />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setAllowDenyForm((f) => ({ ...f, status: e.target.value === "allow" }))}
                >
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowAddAllowDeny(false)}>Cancel</Button>
              <Button onClick={handleAddAllowDeny} disabled={addAllowDenyMutation.isPending || !allowDenyForm.wallet.trim()}>
